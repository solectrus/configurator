import type { Answers } from '@/types/answers'

export const testCases: [string, Answers][] = [
  [
    'local-full',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump'],
      installation_type: 'local',
      linux_machine: 'other',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'senec3',
      wallbox_vendor: 'senec',
      heatpump_access: 'shelly',
      senec_host: '1.2.3.4',
      senec_language: 'it',
      senec_schema: 'http',
      senec_interval: 9,
      shelly_host: '5.6.7.8',
      shelly_interval: 10,
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '1',
      forecast_latitude: 11.1,
      forecast_longitude: 22.2,
      forecast_declination1: 33.3,
      forecast_azimuth1: 0,
      forecast_kwp1: 5.55,
      backup: true,
      installation_date: '2021-01-01',
      admin_password: 'admin-password',
      aws_access_key_id: 'aws-access-key-id',
      aws_secret_access_key: 'aws-secret',
      aws_region: 'eu-central-1',
      aws_bucket: 'my-bucket',
    },
  ],
  [
    'local-full-mqtt',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump', 'car'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'other',
      wallbox_vendor: 'other',
      heatpump_access: 'mqtt',
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '2',
      forecast_latitude: 11.1,
      forecast_longitude: 22.2,
      forecast_declination1: 33.1,
      forecast_azimuth1: 44.1,
      forecast_kwp1: 5.1,
      forecast_declination2: 33.2,
      forecast_azimuth2: 44.2,
      forecast_kwp2: 5.2,
      backup: true,
      admin_password: 'admin-password',
      aws_access_key_id: 'aws-access-key-id',
      aws_secret_access_key: 'aws-secret',
      aws_region: 'eu-central-1',
      aws_bucket: 'my-bucket',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_port: 1883,
      mqtt_ssl: true,
      mqtt_username: 'my-mqtt-username',
      mqtt_password: 'my-mqtt-password',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_wallbox_power: 'foo/0/wallbox',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      mqtt_heatpump_power: 'foo/0/heatpump',
      mqtt_car_battery_soc: 'foo/0/car_battery_soc',
      mqtt_wallbox_car_connected: 'foo/0/wallbox_car_connected',
      wallbox_exclude_from_house_power: 'yes',
      heatpump_exclude_from_house_power: 'yes',
    },
  ],
  [
    'local-full-mqtt-synology',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump'],
      installation_type: 'local',
      linux_machine: 'synology',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'other',
      wallbox_vendor: 'other',
      heatpump_access: 'mqtt',
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '2',
      forecast_latitude: 11.1,
      forecast_longitude: 22.2,
      forecast_declination1: 33.1,
      forecast_azimuth1: 44.1,
      forecast_kwp1: 5.1,
      forecast_declination2: 33.2,
      forecast_azimuth2: 44.2,
      forecast_kwp2: 5.2,
      backup: true,
      admin_password: 'admin-password',
      aws_access_key_id: 'aws-access-key-id',
      aws_secret_access_key: 'aws-secret',
      aws_region: 'eu-central-1',
      aws_bucket: 'my-bucket',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_port: 1883,
      mqtt_ssl: true,
      mqtt_username: 'my-mqtt-username',
      mqtt_password: 'my-mqtt-password',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_wallbox_power: 'foo/0/wallbox',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      mqtt_heatpump_power: 'foo/0/heatpump',
      wallbox_exclude_from_house_power: 'yes',
      heatpump_exclude_from_house_power: 'yes',
    },
  ],
  [
    'local-mqtt-without-wallbox',
    {
      devices: ['inverter', 'battery', 'heatpump'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'senec3',
      heatpump_access: 'mqtt',
      admin_password: 'admin-password',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      mqtt_heatpump_power: 'foo/0/heatpump',
      heatpump_exclude_from_house_power: 'yes',
    },
  ],
  [
    'local-mqtt-with-wallbox',
    {
      devices: ['inverter', 'battery', 'wallbox'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'senec3',
      wallbox_vendor: 'other',
      heatpump_access: 'mqtt',
      admin_password: 'admin-password',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      mqtt_wallbox_power: 'foo/0/wallbox',
      wallbox_exclude_from_house_power: 'yes',
    },
  ],
  [
    'local-mqtt-with-not-excluded-wallbox',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'other',
      wallbox_vendor: 'other',
      heatpump_access: 'mqtt',
      admin_password: 'admin-password',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_wallbox_power: 'foo/0/wallbox',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      mqtt_heatpump_power: 'foo/0/heatpump',
      wallbox_exclude_from_house_power: 'no',
      heatpump_exclude_from_house_power: 'yes',
    },
  ],
  [
    'local-mqtt-shelly',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'other',
      wallbox_vendor: 'other',
      heatpump_access: 'shelly',
      admin_password: 'admin-password',
      shelly_host: '5.6.7.8',
      shelly_interval: 5,
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'neg_pos',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'neg_pos',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_wallbox_power: 'foo/0/wallbox',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
      wallbox_exclude_from_house_power: 'no',
      heatpump_exclude_from_house_power: 'yes',
    },
  ],
  [
    'forecast-three-roofs',
    {
      devices: ['inverter'],
      installation_type: 'local',
      linux_machine: 'other',
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '3',
      forecast_latitude: 11.1,
      forecast_longitude: 22.2,
      forecast_declination1: 33.1,
      forecast_azimuth1: 44.1,
      forecast_kwp1: 5.1,
      forecast_declination2: 33.2,
      forecast_azimuth2: 44.2,
      forecast_kwp2: 5.2,
      forecast_declination3: 33.3,
      forecast_azimuth3: 44.3,
      forecast_kwp3: 5.3,
    },
  ],
  [
    'forecast-four-roofs',
    {
      devices: ['inverter'],
      installation_type: 'local',
      linux_machine: 'other',
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '4',
      forecast_latitude: 11.1,
      forecast_longitude: 22.2,
      forecast_declination1: 33.1,
      forecast_azimuth1: 44.1,
      forecast_kwp1: 5.1,
      forecast_declination2: 33.2,
      forecast_azimuth2: 44.2,
      forecast_kwp2: 5.2,
      forecast_declination3: 33.3,
      forecast_azimuth3: 44.3,
      forecast_kwp3: 5.3,
      forecast_declination4: 33.4,
      forecast_azimuth4: 44.4,
      forecast_kwp4: 5.4,
    },
  ],
  [
    'forecast-solcast',
    {
      devices: ['inverter'],
      installation_type: 'local',
      linux_machine: 'other',
      forecast: 'forecast_solcast',
      forecast_roofs: '1',
      forecast_solcast_api_key: 'solcast-api-key',
      forecast_solcast_id1: 'solcast-id',
    },
  ],
  [
    'local-full-mqtt-flip',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'local',
      linux_machine: 'other',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'other',
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_username: 'my-mqtt-username',
      mqtt_password: 'my-mqtt-password',
      mqtt_inverter_power: 'foo/0/inverter',
      mqtt_house_power: 'foo/0/house',
      mqtt_grid_power: 'foo/0/grid',
      mqtt_grid_power_pos_neg: 'pos_neg',
      mqtt_battery_power: 'foo/0/battery',
      mqtt_battery_power_pos_neg: 'pos_neg',
      mqtt_battery_soc: 'foo/0/soc',
      mqtt_case_temp: 'foo/0/case_temp',
      mqtt_system_status: 'foo/0/system_status',
      mqtt_system_status_ok: 'foo/0/system_status_ok',
      mqtt_grid_export_limit: 'foo/0/grid_export_limit',
    },
  ],
  [
    'local-minimal',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'local',
      linux_machine: 'other',
      app_host: '192.168.108.42',
      app_port: 3001,
      battery_vendor: 'senec3',
      senec_host: '1.2.3.4',
      senec_language: 'it',
      senec_schema: 'http',
      installation_date: '2021-01-02',
      admin_password: 'admin-password',
    },
  ],
  [
    'distributed-local',
    {
      devices: ['inverter', 'battery', 'wallbox'],
      installation_type: 'distributed',
      linux_machine: 'other',
      influx_host: 'solectrus.my-website.de',
      influx_schema: 'https',
      distributed_choice: 'local',
      battery_vendor: 'senec3',
      wallbox_vendor: 'other',
      heatpump_access: 'shelly',
      senec_host: '1.2.3.4',
      senec_language: 'it',
      senec_schema: 'http',
      senec_interval: 5,
      shelly_host: '5.6.7.8',
      shelly_interval: 5,
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_username: 'my-mqtt-username',
      mqtt_password: 'my-mqtt-password',
      mqtt_wallbox_power: 'foo/0/wallbox',
      installation_date: '2021-01-03',
      admin_password: 'admin-password',
    },
  ],
  [
    'distributed-cloud',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'distributed',
      distributed_choice: 'cloud',
      app_host: '42.42.42.42',
      battery_vendor: 'senec3',
      installation_date: '2021-01-04',
      admin_password: 'admin-password',
    },
  ],
  [
    'cloud-senec3',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'cloud',
      app_host: '42.42.42.42',
      battery_vendor: 'senec3',
      senec_username: 'peter@gmx.de',
      senec_password: 'mysecret',
      senec_system_id: '111',
      senec_interval_cloud: 45,
      installation_date: '2021-01-05',
      admin_password: 'admin-password',
    },
  ],
  [
    'cloud-senec4',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'cloud',
      app_host: '42.42.42.42',
      battery_vendor: 'senec4',
      senec_username: 'peter@gmx.de',
      senec_password: 'mysecret',
      senec_system_id: '111',
      senec_interval_cloud: 45,
      installation_date: '2021-01-06',
      admin_password: 'admin-password',
    },
  ],
  [
    'cloud-senec4-wallbox',
    {
      devices: ['inverter', 'battery', 'wallbox'],
      installation_type: 'cloud',
      app_host: '42.42.42.42',
      battery_vendor: 'senec4',
      wallbox_vendor: 'senec',
      senec_username: 'peter@gmx.de',
      senec_password: 'mysecret',
      senec_system_id: '111',
      senec_interval_cloud: 45,
      installation_date: '2021-01-06',
      admin_password: 'admin-password',
    },
  ],
  [
    'cloud-senec4-traefik',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'cloud',
      battery_vendor: 'senec4',
      senec_username: 'peter@gmx.de',
      senec_password: 'mysecret',
      senec_system_id: '111',
      senec_interval_cloud: 45,
      installation_date: '2021-01-06',
      admin_password: 'admin-password',
      traefik: true,
      app_domain: 'solectrus.my-website.de',
    },
  ],
  [
    'cloud-senec4-mfa',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'cloud',
      battery_vendor: 'senec4',
      senec_username: 'peter@gmx.de',
      senec_password: 'mysecret',
      senec_totp_uri:
        'otpauth://totp/SENEC:peter%40gmx.de?secret=XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX&digits=6&algorithm=SHA1&issuer=SENEC&period=30',
      senec_system_id: '111',
      senec_interval_cloud: 60,
      installation_date: '2021-01-06',
      admin_password: 'admin-password',
      traefik: true,
      app_domain: 'solectrus.my-website.de',
    },
  ],
  [
    'partial',
    {
      devices: ['inverter', 'battery'],
      installation_type: 'cloud',
      battery_vendor: 'senec4',
      app_host: '42.42.42.42',
    },
  ],
  [
    'no-devices',
    {
      devices: [],
      installation_type: 'local',
      linux_machine: 'other',
    },
  ],
  [
    'balcony–power-plant',
    {
      devices: ['inverter'],
      installation_type: 'local',
      linux_machine: 'synology',
      app_host: '192.168.108.42',
      app_port: 3001,
      mqtt_host: 'my-mqtt-broker.local',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_username: 'my-mqtt-username',
      mqtt_password: 'my-mqtt-password',
    },
  ],
  [
    'senec-other-wallbox',
    {
      devices: ['inverter', 'battery', 'wallbox', 'car'],
      battery_vendor: 'senec3',
      wallbox_vendor: 'other',
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: 'raspi',
      app_port: 3000,
      senec_host: 'senec',
      senec_interval: 5,
      senec_schema: 'https',
      senec_language: 'de',
      mqtt_host: 'broker',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_wallbox_power: 'wallbox/power',
      mqtt_wallbox_car_connected: 'wallbox/car_connected',
      mqtt_car_battery_soc: 'car_battery_soc',
    },
  ],
  [
    'senec-wallbox-car',
    {
      devices: ['inverter', 'battery', 'wallbox', 'car'],
      battery_vendor: 'senec3',
      wallbox_vendor: 'senec',
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: 'raspi',
      app_port: 3000,
      senec_host: 'senec',
      senec_interval: 5,
      senec_schema: 'https',
      senec_language: 'de',
      mqtt_host: 'broker',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_car_battery_soc: 'car_battery_soc',
    },
  ],
  [
    'mqtt-wallbox-car',
    {
      devices: ['inverter', 'wallbox', 'car'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: 'raspi',
      app_port: 3000,
      senec_host: 'senec',
      senec_interval: 5,
      senec_schema: 'https',
      senec_language: 'de',
      mqtt_host: 'broker',
      mqtt_port: 1883,
      mqtt_ssl: false,
      mqtt_inverter_power: 'inverter/power',
      mqtt_grid_power: 'grid/power',
      mqtt_grid_power_pos_neg: 'pos_neg',
      mqtt_house_power: 'house/power',
      mqtt_wallbox_power: 'wallbox/power',
      wallbox_exclude_from_house_power: 'yes',
      mqtt_wallbox_car_connected: 'wallbox/car_connected',
      mqtt_car_battery_soc: 'car_battery_soc',
    },
  ],
  [
    'local-home-assistant',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump'],
      home_automation: ['home_assistant'],
      installation_type: 'local',
      linux_machine: 'raspberry',
      app_host: '192.168.178.50',
      app_port: 3000,
      forecast: 'forecast_forecast_solar',
      forecast_roofs: '1',
      forecast_latitude: 52.52,
      forecast_longitude: 13.405,
      forecast_declination1: 35,
      forecast_azimuth1: 0,
      forecast_kwp1: 10,
      installation_date: '2024-01-01',
      admin_password: 'admin-password',
    },
  ],
  [
    'local-iobroker',
    {
      devices: ['inverter', 'battery', 'wallbox', 'heatpump', 'car'],
      home_automation: ['iobroker'],
      installation_type: 'local',
      linux_machine: 'synology',
      app_host: '192.168.178.60',
      app_port: 3001,
      forecast: 'forecast_solcast',
      forecast_roofs: '2',
      forecast_solcast_api_key: 'my-solcast-api-key',
      forecast_solcast_id1: '1234-5678-9012-3456',
      forecast_solcast_id2: 'abcd-efgh-ijkl-mnop',
      backup: true,
      aws_access_key_id: 'aws-key',
      aws_secret_access_key: 'aws-secret',
      aws_region: 'eu-central-1',
      aws_bucket: 'my-backup-bucket',
      installation_date: '2024-02-01',
      admin_password: 'admin-password',
    },
  ],
]
